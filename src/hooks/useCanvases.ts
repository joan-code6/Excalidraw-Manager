...
function deriveProjectsFromCanvases(items: ExcalidrawCanvas[]): string[] {
  return Array.from(new Set([DEFAULT_PROJECT, ...items.map((canvas) => canvas.project || DEFAULT_PROJECT)]));
}

function mergeProjectsWithDerived(storedProjects: string[], items: ExcalidrawCanvas[]): string[] {
  return Array.from(
    new Set([
      DEFAULT_PROJECT,
...
function loadCloudBaseline(userId: string): {
  ...
  try {
    ...
    const updatedAtRecord = typeof parsed.updatedAt === 'object' && parsed.updatedAt
...
function saveCloudBaseline(userId: string, ids: string[], updatedAtMap: Map<string, number>) {
  try {
...
export function useCanvases() {
  ...
  useEffect(() => {
    const loadCanvases = () => {
      try {
        ...
        const mergedProjects = mergeProjectsWithDerived(storedProjectsList, normalizedCanvases);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const rawRemoteCanvases = normalizeCanvases(await listUserCanvases(user.$id));
        const rawRemoteIdSet = new Set(rawRemoteCanvases.map((canvas) => canvas.id));

        // Clear tombstones only when a fetch that started after delete-time confirms absence remotely.
        for (const [deletedId, deletedAt] of deleteTombstonesRef.current.entries()) {
        ...
        const remoteCanvases = rawRemoteCanvases.filter(
          (canvas) =>
            !pendingDeleteIdsRef.current.has(canvas.id) && !deleteTombstonesRef.current.has(canvas.id)
        );
        ...
        if (stored) {
          try {
            localCanvases = normalizeCanvases(JSON.parse(stored) as ExcalidrawCanvas[]);
          } catch (err) {
            console.error('Failed to parse local canvases for merge:', err);
          }
        }

        const remoteById = new Map(remoteCanvases.map((canvas) => [canvas.id, canvas]));
        ...
        knownRemoteIdsRef.current = new Set(rawRemoteCanvases.map((canvas) => canvas.id));
        previousCanvasIdsRef.current = remoteCanvases.map((c) => c.id);
        previousCanvasUpdatedAtRef.current = new Map(
          remoteCanvases.map((canvas) => [canvas.id, canvas.updatedAt || 0])
        );
        saveCloudBaseline(user.$id, previousCanvasIdsRef.current, previousCanvasUpdatedAtRef.current);
      } catch (err) {
  ...
  }, [blockCloudSync, canSyncWithDb, remoteRefreshTick, saveCanvases, user?.$id]);
        ...
        for (const canvas of canvases) {
          ...
          if (previousUpdatedAt !== canvasUpdatedAt) {
            const upsertMode: CanvasUpsertMode =
              previousIdSet.has(canvas.id) || previousUpdatedAtMap.has(canvas.id)
                ? 'prefer-update'
                : 'prefer-create';
